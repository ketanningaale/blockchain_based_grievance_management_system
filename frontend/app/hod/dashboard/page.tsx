...
export default function HoDDashboardPage() {
  ...
  const {
    ...
    queryKey: ["grievances", "hod"],
    queryFn:  fetchHoDGrievances,
    staleTime: 30_000,
  });

  const now      = Math.floor(Date.now() / 1000);
  const pending  = grievances.filter((g) => g.status === "AtHoD").length;
  const overdue  = grievances.filter(
    (g) => g.status === "AtHoD" && g.threshold_deadline < now
  ).length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <KpiCard label="Awaiting Action" value={pending}           color="text-amber-600"  iconBg="bg-amber-50"  icon={<Clock className="h-5 w-5 text-amber-500" />} />
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          <p className="text-xs text-gray-400 font-semibold uppercase tracking-wider">
            Grievances escalated to your department
          </p>
          {grievances.map((g) => (
            <GrievanceCard
              key={g.id}
              grievance={g}
              detailBase="/hod/grievance"
              showStudent
            />
          ))}
        </div>
...
function EmptyState() {
  return (
    <div className="card flex flex-col items-center justify-center py-20 text-center px-6">
      ...
      <p className="text-base font-semibold text-gray-700">No grievances forwarded to you yet</p>
      <p className="text-sm text-gray-400 mt-1">Grievances escalated from the committee will appear here.</p>
    </div>
  );
}
